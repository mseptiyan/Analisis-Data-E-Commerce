...
@st.cache_data
def load_data():
    return pd.read_csv("dashboard/rfm_analysis_results.csv")
...
col1.metric("Jumlah Pelanggan", df.shape[0])
col2.metric("Rata-rata Recency", round(df["Recency"].mean(), 2))
col3.metric("Rata-rata Frequency", round(df["Frequency"].mean(), 2))
...
# Recency
sns.histplot(df["Recency"], bins=50, kde=True, color="blue", ax=axes[0])
axes[0].set_title("Distribusi Recency")

# Frequency
sns.histplot(df["Frequency"], bins=50, kde=True, color="green", ax=axes[1])
axes[1].set_title("Distribusi Frequency")

# Monetary
sns.histplot(df["Monetary"], bins=50, kde=True, color="red", ax=axes[2])
...
# Load Data Produk dan Order Items
@st.cache_data
...
def load_payment_data():
    ...
    return order_payments['payment_type'].value_counts()
